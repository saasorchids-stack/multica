...
func agentToResponse(a db.Agent) AgentResponse {
	...
	// Decrypt env values (no-op if ENCRYPTION_KEY not set or values not encrypted).
	if decrypted, err := crypto.DecryptMap(customEnv); err == nil {
		customEnv = decrypted
	}
...
func (h *Handler) CreateAgent(w http.ResponseWriter, r *http.Request) {
	...
	// Encrypt custom_env values at rest.
	envToStore := req.CustomEnv
	if envToStore != nil {
		if encrypted, err := crypto.EncryptMap(envToStore); err == nil {
			envToStore = encrypted
		} else {
			slog.Warn("failed to encrypt custom_env", "error", err)
		}
	}

	ce, _ := json.Marshal(envToStore)
...
func (h *Handler) UpdateAgent(w http.ResponseWriter, r *http.Request) {
	...
	if req.CustomEnv != nil {
		...
		if encrypted, err := crypto.EncryptMap(envToStore); err == nil {
			...
			slog.Warn("failed to encrypt custom_env on update", "error", err)
		}
		ce, _ := json.Marshal(envToStore)
		params.CustomEnv = ce
	}
