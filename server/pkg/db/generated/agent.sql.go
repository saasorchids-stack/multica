...
WHERE (status = 'queued' AND created_at < now() - make_interval(secs => $1::double precision))
   OR (status = 'dispatched' AND dispatched_at < now() - make_interval(secs => $2::double precision))
   OR (status = 'running' AND started_at < now() - make_interval(secs => $3::double precision))
RETURNING id, agent_id, issue_id
`

type FailStaleTasksParams struct {
	QueuedTimeoutSecs   float64 `json:"queued_timeout_secs"`
...
func (q *Queries) FailStaleTasks(ctx context.Context, arg FailStaleTasksParams) ([]FailStaleTasksRow, error) {
	rows, err := q.db.Query(ctx, failStaleTasks, arg.QueuedTimeoutSecs, arg.DispatchTimeoutSecs, arg.RunningTimeoutSecs)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
