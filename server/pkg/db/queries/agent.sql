...
-- Fails tasks stuck in queued/dispatched/running beyond the given thresholds.
-- Handles orphaned tasks: no daemon claimed them, agent process hung, or
-- daemon failed to report completion.
UPDATE agent_task_queue
SET status = 'failed', completed_at = now(), error = CASE
    WHEN status = 'queued' THEN 'no daemon claimed this task — is the daemon running?'
    ELSE 'task timed out'
  END
WHERE (status = 'queued' AND created_at < now() - make_interval(secs => @queued_timeout_secs::double precision))
   OR (status = 'dispatched' AND dispatched_at < now() - make_interval(secs => @dispatch_timeout_secs::double precision))
