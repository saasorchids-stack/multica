...
const (
	...
	// queuedTimeoutSeconds fails tasks stuck in 'queued' beyond this.
	// If no daemon claims a task within 10 minutes, something is wrong
	// (daemon not running, runtime offline, etc.).
	queuedTimeoutSeconds = 600.0
...
func sweepStaleTasks(ctx context.Context, queries *db.Queries, bus *events.Bus) {
	failedTasks, err := queries.FailStaleTasks(ctx, db.FailStaleTasksParams{
		QueuedTimeoutSecs:   queuedTimeoutSeconds,
