...
func NewRouter(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus) chi.Router {
	...
	r.Use(middleware.RateLimit(100, 20)) // 100 burst, 20 req/s per key
	...
	// Auth (public) — strict rate limit: 5 burst, 1 req/s per IP
	authLimiter := middleware.StrictRateLimit(5, 1)
	r.With(authLimiter).Post("/auth/send-code", h.SendCode)
	r.With(authLimiter).Post("/auth/verify-code", h.VerifyCode)
